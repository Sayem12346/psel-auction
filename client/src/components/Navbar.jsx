  ...
  return (
    <>
      <style>{`
        ...
        .bottom-nav-item:hover svg{stroke:#00CF33}
      ...
      <nav style={{ background: '#0d0d1a', borderBottom: '1px solid #1e1e3a', padding: '0 16px', height: 60, display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 100, fontFamily: 'Rajdhani, sans-serif' }}>
        <Link to="/" style={{ display: 'flex', alignItems: 'center', gap: 8, textDecoration: 'none', flexShrink: 0 }}>
          <div style={{ width: 34, height: 34, background: 'linear-gradient(135deg, #00CF33, #00CF33)', borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 900, fontSize: 15, color: '#fff', fontFamily: 'Orbitron' }}>P</div>
          <div>
            <div style={{ color: '#fff', fontWeight: 900, fontSize: 14, fontFamily: 'Orbitron', lineHeight: 1 }}>PSEL</div>
            <div style={{ color: '#00CF33', fontSize: 7, letterSpacing: 2 }}>ESPORTS AUCTION</div>
          </div>
        </Link>

        <div className="desktop-nav" style={{ display: 'flex', gap: 2, alignItems: 'center', flex: 1, justifyContent: 'center' }}>
          {navLinks.map(([path, label]) => (
            <Link key={path} to={path} className="nav-link" style={{ color: location.pathname === path ? '#00CF33' : '#9ca3af', textDecoration: 'none', fontSize: 12, fontWeight: 600, padding: '5px 10px', borderRadius: 6, letterSpacing: 0.5, whiteSpace: 'nowrap', transition: 'all 0.2s', background: location.pathname === path ? '#00CF3311' : 'transparent' }}>{label}</Link>
          ))}
        </div>
          ...
          {owner ? (
            <div ref={dropRef} style={{ position: 'relative' }}>
              <button onClick={() => setDropdown(!dropdown)} style={{ background: '#1e1e3a', border: '1px solid #2e2e4a', borderRadius: 10, padding: '5px 10px', display: 'flex', alignItems: 'center', gap: 6, cursor: 'pointer', color: '#fff', fontFamily: 'Rajdhani' }}>
                <div style={{ width: 26, height: 26, borderRadius: 6, background: '#00CF3322', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', flexShrink: 0 }}>
                  {owner.profileImage ? <img src={owner.profileImage} alt={owner.name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <span style={{ color: '#00CF33', fontWeight: 900, fontSize: 12 }}>{owner.name?.[0]}</span>}
                </div>
              ...
              {dropdown && (
                <div style={{ position: 'absolute', right: 0, top: '110%', background: '#0d0d1a', border: '1px solid #1e1e3a', borderRadius: 12, minWidth: 200, zIndex: 200, animation: 'slideDown 0.15s ease', boxShadow: '0 8px 32px #00000088' }}>
                  ...
                  <button onClick={logout} style={{ width: '100%', padding: '11px 16px', color: '#00CF33', background: 'transparent', border: 'none', textAlign: 'left', cursor: 'pointer', fontSize: 13, fontWeight: 600 }}>Logout</button>
                </div>
              )}
            </div>
          ) : (
            <Link to="/owner/login" className="desktop-nav" style={{ background: 'linear-gradient(135deg, #00CF33, #00CF33)', color: '#fff', textDecoration: 'none', padding: '7px 12px', borderRadius: 8, fontSize: 11, fontWeight: 700, letterSpacing: 0.5, whiteSpace: 'nowrap' }}>OWNER LOGIN</Link>
          )}
        </div>
      </nav>
        ...
        {bottomNav.map(({ path, label, icon }) => {
          ...
          return (
            <Link key={path} to={path} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 4, textDecoration: 'none', padding: '8px 4px' }}>
              {label === 'Auction' ? (
                <div style={{ width: 44, height: 44, borderRadius: '50%', background: isActive ? 'linear-gradient(135deg, #00CF33, #00CF33)' : '#1e1e3a', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 2, border: isActive ? 'none' : '1px solid #2e2e4a' }}>
                ...
                <svg width="22" height="22" fill="none" viewBox="0 0 24 24" stroke={isActive ? '#00CF33' : '#6b7280'} strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d={icon} /></svg>
              )}
              <span style={{ fontSize: 10, fontWeight: 600, color: isActive ? '#00CF33' : '#6b7280', fontFamily: 'Rajdhani', letterSpacing: 0.5 }}>{label}</span>
            </Link>
          );
        })}
      </div>
