...
export default function LivePage() {
  ...
  useEffect(() => {
    ...
    socket.on('playerSold', (data) => { setPopup({ type: 'sold', winner: data.winner, amount: data.amount, player: data.player }); setTimeout(() => setPopup(null), 4000); fetchData(); });
    socket.on('bidError', (data) => { setMsg(data.message); setTimeout(() => setMsg(''), 3000); });
    socket.on('playerUnsold', (data) => { setPopup({ type: 'unsold', player: data.player }); setTimeout(() => setPopup(null), 4000); fetchData(); });
  ...
  const timerColor = timer <= 5 ? '#00CF33' : timer <= 10 ? '#f59e0b' : '#00ff88';
    ...
    <div style={{ minHeight: '100vh', background: '#080812', fontFamily: "'Rajdhani', sans-serif", color: '#fff' }}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        ::-webkit-scrollbar{width:3px} ::-webkit-scrollbar-thumb{background:#00CF3344;border-radius:2px}
        @keyframes pulse{0%,100%{opacity:1}50%{opacity:0.4}}
        @keyframes glow{0%,100%{box-shadow:0 0 10px #00CF3333}50%{box-shadow:0 0 20px #00CF33aa}}
        ...
        input:focus{outline:none;border-color:#00CF33 !important}
        .owner-row:hover{background:#00CF3311 !important}
      ...
      {popup && (
        <div style={{ position: 'fixed', inset: 0, background: '#000000cc', zIndex: 999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
          <div style={{ background: popup.type === 'sold' ? 'linear-gradient(135deg, #0a1f0a, #0d0d1a)' : 'linear-gradient(135deg, #1f0a0a, #0d0d1a)', border: `2px solid ${popup.type === 'sold' ? '#00ff88' : '#00CF33'}`, borderRadius: 24, padding: '32px 40px', textAlign: 'center', animation: 'popIn 0.4s cubic-bezier(0.175,0.885,0.32,1.275)', boxShadow: `0 0 80px ${popup.type === 'sold' ? '#00ff8833' : '#00CF3333'}`, maxWidth: 340, width: '100%' }}>
            <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 16 }}>
              <div style={{ width: 80, height: 80, borderRadius: 16, background: '#1e1e3a', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', border: `2px solid ${popup.type === 'sold' ? '#00ff8844' : '#00CF3344'}` }}>
                {popup.player?.playerImage ? <img src={popup.player.playerImage} alt={popup.player.name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <span style={{ fontSize: 32, fontWeight: 900, color: popup.type === 'sold' ? '#00ff88' : '#00CF33', fontFamily: 'Orbitron' }}>{popup.player?.name?.[0] || '?'}</span>}
              </div>
            </div>
            <div style={{ color: popup.type === 'sold' ? '#00ff88' : '#00CF33', fontSize: 32, fontWeight: 900, fontFamily: 'Orbitron', letterSpacing: 4, marginBottom: 8 }}>{popup.type === 'sold' ? 'SOLD' : 'UNSOLD'}</div>
            <div style={{ color: '#fff', fontSize: 18, fontWeight: 700, marginBottom: 4 }}>{popup.player?.name}</div>
            <div style={{ display: 'flex', gap: 6, justifyContent: 'center', marginBottom: 12, flexWrap: 'wrap' }}>
              {popup.player?.role && <span style={{ background: '#00CF3322', border: '1px solid #00CF3344', padding: '2px 8px', borderRadius: 4, fontSize: 11, color: '#00CF33', fontWeight: 700 }}>{popup.player.role}</span>}
            ...
            <div style={{ borderTop: `1px solid ${popup.type === 'sold' ? '#00ff8822' : '#00CF3322'}`, paddingTop: 12, marginTop: 4 }}>
              {popup.type === 'sold' ? (
                <>
                ...
                <>
                  ...
                  <div style={{ color: '#00CF33', fontSize: 22, fontWeight: 900, fontFamily: 'Orbitron' }}>৳{popup.player?.basePrice?.toLocaleString()}</div>
                </>
              )}
            </div>
          </div>
        ...
        <>
          ...
          <div className="desktop-3col" style={{ display: 'grid', gridTemplateColumns: '180px 1fr 180px', gap: 10, maxWidth: 1400, margin: '0 auto', padding: '10px 16px' }}>
            ...
            <div style={{ background: '#0d0d1a', border: '1px solid #1e1e3a', borderRadius: 12, overflow: 'hidden', height: 'fit-content' }}>
              ...
              {leftOwners.map((o, i) => (
                <div key={o._id} className="owner-row" style={{ padding: '6px 10px', borderBottom: '1px solid #1e1e3a11', display: 'flex', alignItems: 'center', gap: 6, background: o.name === currentLeader ? '#00CF3311' : 'transparent', cursor: 'default' }}>
                  ...
                  <div style={{ width: 22, height: 22, borderRadius: '50%', background: '#1e1e3a', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', flexShrink: 0, border: o.name === currentLeader ? '1px solid #00ff88' : 'none' }}>
                    {o.profileImage ? <img src={o.profileImage} alt={o.name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <span style={{ fontSize: 9, fontWeight: 900, color: '#00CF33' }}>{o.name[0]}</span>}
                  </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'center' }}>
                <div style={{ background: '#00CF3322', border: '1px solid #00CF33', borderRadius: 20, padding: '3px 16px', display: 'flex', alignItems: 'center', gap: 6, animation: 'glow 2s infinite' }}>
                  <div style={{ width: 7, height: 7, borderRadius: '50%', background: '#00CF33', animation: 'pulse 1s infinite' }} />
                  <span style={{ color: '#00CF33', fontSize: 11, fontWeight: 700, letterSpacing: 2, fontFamily: 'Orbitron' }}>LIVE AUCTION</span>
                </div>
              </div>

              {/* Player card */}
              <div style={{ background: 'linear-gradient(135deg, #0d0d1a, #1a0a2e)', border: '1px solid #00CF3333', borderRadius: 14, overflow: 'hidden', position: 'relative' }}>
                <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: 3, background: 'linear-gradient(90deg, #00CF33, #00CF33, #00CF33)' }} />
                <div style={{ display: 'flex', gap: 16, padding: 16, alignItems: 'flex-start' }}>
                  <div style={{ width: 120, height: 145, borderRadius: 10, background: '#1e1e3a', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, overflow: 'hidden', border: '2px solid #00CF3333' }}>
                    {currentPlayer?.playerImage ? <img src={currentPlayer.playerImage} alt={currentPlayer.name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <span style={{ fontSize: 44, fontWeight: 900, color: '#00CF3388', fontFamily: 'Orbitron' }}>{currentPlayer?.name?.[0]}</span>}
                  </div>
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ display: 'flex', gap: 6, marginBottom: 10, flexWrap: 'wrap' }}>
                      {currentPlayer?.role && <span style={{ background: '#00CF3322', border: '1px solid #00CF3344', padding: '2px 8px', borderRadius: 4, fontSize: 10, color: '#00CF33', fontWeight: 700 }}>{currentPlayer.role}</span>}
                ...
                {owner ? (
                  <div style={{ padding: '10px 16px', borderTop: '1px solid #1e1e3a', background: '#0a0a16' }}>
                    {msg && <div style={{ background: '#00CF3322', borderRadius: 6, padding: '6px 10px', color: '#00CF33', fontSize: 11, marginBottom: 8, textAlign: 'center' }}>{msg}</div>}
                    ...
                    <div style={{ display: 'flex', gap: 8 }}>
                      ...
                      <button onClick={() => placeBid()} style={{ background: 'linear-gradient(135deg, #00CF33, #00CF33)', color: '#fff', border: 'none', padding: '8px 16px', borderRadius: 8, cursor: 'pointer', fontSize: 12, fontWeight: 700, letterSpacing: 1 }}>PLACE BID</button>
                    </div>
                  </div>
                ) : (
                  <div style={{ padding: '10px 16px', borderTop: '1px solid #1e1e3a', background: '#0a0a16', textAlign: 'center' }}>
                    ...
                    <Link to="/owner/login" style={{ color: '#00CF33', fontWeight: 700, fontSize: 12, textDecoration: 'none' }}>Login as Owner</Link>
                  </div>
                )}
              </div>
                    ...
                    <div key={i} style={{ padding: '8px 14px', borderBottom: '1px solid #1e1e3a11', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        <div style={{ width: 24, height: 24, borderRadius: '50%', background: '#1e1e3a', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                          <span style={{ fontSize: 10, fontWeight: 900, color: '#00CF33' }}>{b.owner[0]}</span>
                        </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                <div style={{ background: '#0d0d1a', border: '1px solid #00ff8822', borderRadius: 12, overflow: 'hidden' }}>
                  <div style={{ padding: '8px 12px', borderBottom: '1px solid #1e1e3a', display: 'flex', justifyContent: 'space-between' }}>
                    ...
                    <Link to="/players" style={{ color: '#00CF33', fontSize: 10, textDecoration: 'none' }}>All</Link>
                  </div>
                ...
                <div style={{ background: '#0d0d1a', border: '1px solid #00CF3322', borderRadius: 12, overflow: 'hidden' }}>
                  <div style={{ padding: '8px 12px', borderBottom: '1px solid #1e1e3a' }}>
                    <span style={{ color: '#00CF33', fontWeight: 700, fontSize: 11 }}>UNSOLD ({unsoldPlayers.length})</span>
                  </div>
                  <div style={{ maxHeight: 150, overflowY: 'auto' }}>
                    {unsoldPlayers.slice(0,6).map(p => (
                      <div key={p._id} style={{ padding: '6px 10px', borderBottom: '1px solid #1e1e3a11', display: 'flex', alignItems: 'center', gap: 6 }}>
                        <div style={{ width: 22, height: 22, borderRadius: 4, background: '#1e1e3a', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', flexShrink: 0 }}>
                          {p.playerImage ? <img src={p.playerImage} alt={p.name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <span style={{ fontSize: 9, fontWeight: 900, color: '#00CF33' }}>{p.name[0]}</span>}
                        </div>
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ color: '#fff', fontSize: 10, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{p.name}</div>
                          <div style={{ color: '#00CF33', fontSize: 9 }}>No Bids</div>
                        </div>
                      </div>
                    ))}
                  </div>
            ...
            <div style={{ background: '#0d0d1a', border: '1px solid #1e1e3a', borderRadius: 12, overflow: 'hidden', height: 'fit-content' }}>
              <div style={{ padding: '8px 12px', borderBottom: '1px solid #1e1e3a', color: '#f59e0b', fontWeight: 700, fontSize: 11, letterSpacing: 1 }}>OWNERS</div>
              {rightOwners.map((o, i) => (
                <div key={o._id} className="owner-row" style={{ padding: '6px 10px', borderBottom: '1px solid #1e1e3a11', display: 'flex', alignItems: 'center', gap: 6, background: o.name === currentLeader ? '#00CF3311' : 'transparent' }}>
          ...
          <div className="mobile-sections" style={{ padding: '12px' }}>
            <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 10 }}>
              <div style={{ background: '#00CF3322', border: '1px solid #00CF33', borderRadius: 20, padding: '3px 16px', display: 'flex', alignItems: 'center', gap: 6 }}>
                <div style={{ width: 7, height: 7, borderRadius: '50%', background: '#00CF33', animation: 'pulse 1s infinite' }} />
                <span style={{ color: '#00CF33', fontSize: 11, fontWeight: 700, letterSpacing: 2, fontFamily: 'Orbitron' }}>LIVE AUCTION</span>
              </div>
            </div>
            <div style={{ background: 'linear-gradient(135deg, #0d0d1a, #1a0a2e)', border: '1px solid #00CF3333', borderRadius: 14, overflow: 'hidden', marginBottom: 10 }}>
              <div style={{ height: 3, background: 'linear-gradient(90deg, #00CF33, #00CF33)' }} />
              <div style={{ display: 'flex', gap: 12, padding: 14 }}>
                <div style={{ width: 100, height: 120, borderRadius: 10, background: '#1e1e3a', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, overflow: 'hidden' }}>
                  {currentPlayer?.playerImage ? <img src={currentPlayer.playerImage} alt={currentPlayer.name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <span style={{ fontSize: 36, fontWeight: 900, color: '#00CF3388', fontFamily: 'Orbitron' }}>{currentPlayer?.name?.[0]}</span>}
                </div>
                <div style={{ flex: 1 }}>
              ...
              {owner ? (
                <div style={{ padding: '10px 14px', borderTop: '1px solid #1e1e3a', background: '#0a0a16' }}>
                  {msg && <div style={{ background: '#00CF3322', borderRadius: 6, padding: '6px', color: '#00CF33', fontSize: 11, marginBottom: 8, textAlign: 'center' }}>{msg}</div>}
                  <div style={{ color: '#6b7280', fontSize: 10, marginBottom: 6 }}>Balance: <span style={{ color: '#00ff88', fontWeight: 700 }}>৳{myOwner?.availableCoins?.toLocaleString()}</span></div>
                  ...
                  <div style={{ display: 'flex', gap: 8 }}>
                    ...
                    <button onClick={() => placeBid()} style={{ background: 'linear-gradient(135deg, #00CF33, #00CF33)', color: '#fff', border: 'none', padding: '8px 14px', borderRadius: 8, cursor: 'pointer', fontSize: 12, fontWeight: 700 }}>BID</button>
                  </div>
                </div>
              ) : (
                <div style={{ padding: '10px', borderTop: '1px solid #1e1e3a', background: '#0a0a16', textAlign: 'center' }}>
                  <Link to="/owner/login" style={{ color: '#00CF33', fontWeight: 700, fontSize: 12, textDecoration: 'none' }}>Login to Place Bid</Link>
                </div>
              )}
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 10 }}>
              <div style={{ background: '#0d0d1a', border: '1px solid #1e1e3a', borderRadius: 10, overflow: 'hidden' }}>
                ...
                <div style={{ maxHeight: 200, overflowY: 'auto' }}>
                  {owners.map((o, i) => (
                    <div key={o._id} style={{ padding: '5px 8px', borderBottom: '1px solid #1e1e3a11', display: 'flex', alignItems: 'center', gap: 5, background: o.name === currentLeader ? '#00CF3311' : 'transparent' }}>
                      ...
                      <div style={{ width: 18, height: 18, borderRadius: '50%', background: '#1e1e3a', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', flexShrink: 0 }}>
                        {o.profileImage ? <img src={o.profileImage} alt={o.name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <span style={{ fontSize: 8, fontWeight: 900, color: '#00CF33' }}>{o.name[0]}</span>}
                      </div>
                      <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
              ...
              <div style={{ background: '#0d0d1a', border: '1px solid #00CF3322', borderRadius: 10, overflow: 'hidden' }}>
                <div style={{ padding: '6px 10px', borderBottom: '1px solid #1e1e3a', color: '#00CF33', fontSize: 10, fontWeight: 700 }}>UNSOLD ({unsoldPlayers.length})</div>
                <div style={{ maxHeight: 120, overflowY: 'auto' }}>
                  {unsoldPlayers.map(p => (
                    <div key={p._id} style={{ padding: '5px 8px', borderBottom: '1px solid #1e1e3a11', display: 'flex', gap: 5, alignItems: 'center' }}>
                      <div style={{ width: 18, height: 18, borderRadius: 3, background: '#1e1e3a', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', flexShrink: 0 }}>
                        {p.playerImage ? <img src={p.playerImage} style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <span style={{ fontSize: 8, color: '#00CF33', fontWeight: 900 }}>{p.name[0]}</span>}
                      </div>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ color: '#fff', fontSize: 9, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{p.name}</div>
                        <div style={{ color: '#00CF33', fontSize: 8 }}>No Bids</div>
                      </div>
                    </div>
                  ))}
                </div>
