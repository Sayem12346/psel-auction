    ...
    <div style={{ minHeight: '100vh', background: '#080812', fontFamily: "'Rajdhani', sans-serif", color: '#fff', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
        <div style={{ width: '100%', maxWidth: 400 }}>
          <div style={{ background: 'linear-gradient(135deg, #0d0d1a, #1a0a2e)', border: '1px solid #8b5cf633', borderRadius: 20, padding: 36, position: 'relative', overflow: 'hidden' }}>
            ...
            {msg && <div style={{ background: '#ef444422', border: '1px solid #ef444444', borderRadius: 8, padding: '10px 16px', color: '#ef4444', fontSize: 13, marginBottom: 16, textAlign: 'center' }}>{msg}</div>}
            ...
            <button onClick={handleLogin} disabled={loading} style={{ width: '100%', background: 'linear-gradient(135deg, #8b5cf6, #6d28d9)', color: '#fff', border: 'none', padding: '14px', borderRadius: 10, fontSize: 15, fontWeight: 700, cursor: loading ? 'not-allowed' : 'pointer', letterSpacing: 1, opacity: loading ? 0.7 : 1 }}>
              {loading ? 'LOGGING IN...' : 'LOGIN'}
            </button>

            <div style={{ textAlign: 'center', marginTop: 16 }}>
              <Link to="/live" style={{ color: '#8b5cf6', fontSize: 12, textDecoration: 'none' }}>Watch Live Auction without login</Link>
            </div>
          </div>
        </div>
      </div>
