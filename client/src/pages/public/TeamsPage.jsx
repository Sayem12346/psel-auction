              ...
              <div style={{ padding: 20 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 16 }}>
                  <div style={{ position: 'relative' }}>
                    <div style={{ width: 52, height: 52, borderRadius: 12, background: '#1e1e3a', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', border: '2px solid #00CF3333' }}>
                      {o.profileImage ? <img src={o.profileImage} alt={o.name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <span style={{ fontSize: 20, fontWeight: 900, color: '#00CF33' }}>{o.name[0]}</span>}
                    </div>
                    <div style={{ position: 'absolute', bottom: -4, right: -4, background: '#00CF33', borderRadius: '50%', width: 20, height: 20, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, fontWeight: 900, color: '#fff' }}>{i + 1}</div>
                  </div>
                  <div style={{ flex: 1 }}>
                    ...
                    {o.team.map((p, j) => (
                      <div key={p._id || j} style={{ display: 'flex', alignItems: 'center', gap: 10, background: '#1e1e3a44', borderRadius: 10, padding: '10px 12px' }}>
                        ...
                        <div style={{ width: 36, height: 36, borderRadius: 8, background: '#1e1e3a', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', flexShrink: 0 }}>
                          {p.playerImage ? <img src={p.playerImage} alt={p.name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <span style={{ fontSize: 14, fontWeight: 900, color: '#00CF33' }}>{p.name?.[0] || 'P'}</span>}
                        </div>
                        <div style={{ flex: 1 }}>
